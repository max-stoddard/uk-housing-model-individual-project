...
from scripts.python.helpers.was.plotting import apply_axis_grid
from scripts.python.helpers.was.statistics import mean_variance_skew
from scripts.python.calibration.was import btl_probability_per_income_percentile_bin
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare WAS BTL probability by income percentile bins."
    )
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Optional output directory for generated stats/plots and comparison CSVs.",
    )
    args = parser.parse_args()

    output_dir = args.output_dir or get_output_dir(__file__)
    root_dir = args.output_dir or get_project_root(__file__)
    ...
    wave_3_output = btl_probability_per_income_percentile_bin.run_btl_probability_per_income_percentile_bin(
        WAVE_3_DATA
    )
    ...
    round_8_output = btl_probability_per_income_percentile_bin.run_btl_probability_per_income_percentile_bin(
        ROUND_8_DATA
    )
