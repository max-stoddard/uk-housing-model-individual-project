...
# Filter average data
averages = averages[(averages["# Date"] >= datetime(2003, 3, 15)) & (averages["# Date"] < datetime(2015, 3, 15))]
...
for chunk in pd.read_csv(root + r"\New Zoopla\B Raw Listings (collation).csv", chunksize=chunk_size,
    ...
    chunk = chunk[(chunk["LATEST SOLD"] >= datetime(2003, 4, 15)) & (chunk["LATEST SOLD"] <= datetime(2015, 3, 15))]
    chunk = chunk[(chunk["CREATED"] >= datetime(2003, 4, 15)) & (chunk["CREATED"] <= datetime(2015, 3, 15))]
