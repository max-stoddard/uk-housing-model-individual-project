...
- `dashboard/server/index.ts`: API route registration (`/healthz`, `/api/versions`, `/api/parameter-catalog`, `/api/git-stats`, `/api/compare`, `/api/results/*`).
- `dashboard/server/lib/service.ts`: Comparison engine and payload builders for versioned input-data compare.
- `dashboard/server/lib/results.ts`: Phase 1 Results explorer discovery/parsing/KPI/overlay payload builders.
...
- API endpoints:
  ...
  - `GET /api/results/runs/:runId`
  - `GET /api/results/runs/:runId/files`
  - `GET /api/results/runs/:runId/series?indicator=<id>&smoothWindow=<0|3|12>`
  - `GET /api/results/compare?runIds=<csv>&indicatorIds=<csv>&window=tail120|full&smoothWindow=<0|3|12`
...
- Dev-only page visibility behavior:
  - `Experiments` navigation entry and route are dev-only.
  - when previewing non-dev in local development, `Experiments` is hidden and direct route access redirects to `/`.
  - `Run Model` is visible in both dev and non-dev environments and serves a Phase 1 read-only Results Explorer (no execution controls yet), including:
    - run inventory sorted by `modifiedAt` descending,
    - run health status badges (`complete`, `partial`, `invalid`),
    - overlay charts with max 5 selected runs and shared model-time axis (gaps for missing points),
    - smoothing toggle (`off`, `3`, `12`) and compare window mode (`post200`, `tail120`, `full`) with `post200` as default,
    - KPI summary cards (`tail_120` latest/mean/yoy),
    - run file manifest with coverage states (`supported`, `empty`, `unsupported`, `error`).
